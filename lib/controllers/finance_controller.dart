...
class FinanceController extends GetxController {
  ...
  Future<void> saveFinance(String eventId, FinanceModel financeData) async {
    try {
      ...
      // In saveFinance() — swap and improve:
      Get.back();
      await Future.delayed(const Duration(milliseconds: 300));
      Get.snackbar(
        'Budget Saved',
        'Finance details have been saved successfully',
        snackPosition: SnackPosition.BOTTOM,
...
Future<void> closeBudget(String eventId) async {
  try {
    ...
    // Re-fetch the event so selectedEvent reflects budgetClosed: true
    final eventController = Get.find<EventController>();
    final updated = await _firestoreService.getEventById(eventId);
    if (updated != null) {
      eventController.selectedEvent.value = updated;

      // Also update the event in the events list so the list screen
      // shows the correct icon without needing a full reload
      final index = eventController.events
          .indexWhere((e) => e.id == eventId);
      if (index != -1) {
        eventController.events[index] = updated;
      }
    }

    Get.snackbar(
      'Budget Closed',
      'The budget has been finalized and locked',
      snackPosition: SnackPosition.BOTTOM,
      backgroundColor: const Color(0xFF0F6E56),
      colorText: Colors.white,
      duration: const Duration(seconds: 3),
    ...
    Get.snackbar('Error', 'Could not close budget: $e',
        snackPosition: SnackPosition.BOTTOM);
  }
}
